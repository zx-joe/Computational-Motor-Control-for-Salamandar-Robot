...
def simulation(
    ...
    for iteration in sim.iterator(show_progress=True):
        ...
        position_head=data.sensors.gps.urdf_position(iteration=iteration, link_i=0)
        head_x=position_head[0]
        
        try:
            if head_x<=-2.2 and sim_parameters.transition==['walk','swim']:
                sim_parameters.pattern='swim'
                sim_parameters.drive=4
                network.robot_parameters.update(sim_parameters)
                
            if head_x>=-1.2 and sim_parameters.transition==['swim','walk'] :
                sim_parameters.pattern='walk'
                sim_parameters.drive=2.
                network.robot_parameters.update(sim_parameters)

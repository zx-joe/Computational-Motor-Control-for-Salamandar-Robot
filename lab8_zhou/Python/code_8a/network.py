...
def motor_output(phases, amplitudes, iteration=None):
    ...
    for i in range(14):
        if i<10:
            q[i]=r[i]*(1+np.cos(theta[i]))-r[i+10]*(1+np.cos(theta[i+10]))
        elif np.sum(abs(r[20:]))>0:
            q[i]=-theta[i+10]
            q[i]=r[i+10]*(1+np.cos(theta[i+10]))
...
class SalamandraNetwork:
    ...
    def outputs(self, iteration=None):
        ...
        #temp_output=self.state.amplitudes*(1+np.cos(self.state.phases))
        temp_output=self.state.amplitudes(iteration=iteration)*(1+np.cos(self.state.phases(iteration=iteration)))
        return temp_output
